...
function Sparkline({values, color}: {values:number[]; color:string}) {
  ...
  const w=280,h=60,pad=6,max=Math.max(...values,1),min=Math.min(...values,0),range=(max-min)||1
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        <header className="page-header">
          <div>
            <h1 className="page-title">Analises{tid?' - Ativo':''}</h1>
            <p className="page-subtitle">{items.length} analise{items.length!==1?'s':''}{tid?' deste ativo':''}</p>
          </div>
        ...
        <div className="page-body">
          {items.length===0 ? (
            <div className="empty-state">
              ...
              <div className="empty-title">Nenhuma analise ainda</div>
              <div className="empty-text">Importe laudos DGA para ver os gases e o diagnostico DUVAL</div>
            </div>
          ) : (
            <>
              {tid && items.length>=2 && (
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:'14px',fontWeight:'600',marginBottom:'4px'}}>
                          {tid?(a.transformers?.identificacao||a.transformers?.numero_serie||'Transformador'):(
                            <button style={{background:'none',border:'none',color:'inherit',cursor:'pointer',fontSize:'14px',fontWeight:'600',padding:0}} onClick={()=>router.push('/analyses?transformer='+a.transformer_id)}>
                      ...
                      {GASES.map((g,i)=>{
                        ...
                        return (
                          <div key={g} className="gas-cell" style={hi?{borderColor:'rgba(239,68,68,.3)'}:{}}>
                            ...
                            {delta!=null&&delta!==0&&<div style={{fontSize:'9px',marginTop:'3px',color:delta>0?'#f87171':'#10b981',fontWeight:'600'}}>{delta>0?'+':''}{delta.toFixed(0)}</div>}
                          </div>
                        )
                      })}
                    </div>

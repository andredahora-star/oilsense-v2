...
export default function LubeImportPage() {
  ...
  async function processFiles(files: FileList) {
    ...
    for (const item of pdfs) {
      ...
      try {
        ...
        const d = await fetch('/api/lube-import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pdf_base64:b64,subscription_id:id})}).then(r=>r.json())
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        <header className="page-header">
          <div><h1 className="page-title">Importar Laudos — Óleo Lubrificante</h1><p className="page-subtitle">PDFs de redutores extraídos pelo Claude IA automaticamente</p></div>
        </header>
        <div className="page-body" style={{maxWidth:'720px'}}>
          <label className="upload-zone" style={{display:'block',opacity:uploading?0.6:1,cursor:uploading?'not-allowed':'pointer'}}>
            {uploading?(
              ...
              <>
                ...
                <div style={{fontSize:'13px',color:'var(--text-muted)'}}>Laudos de viscosidade, TAN, água, ISO 4406, espectrometria de metais</div>
          ...
          {done>0&&<div className="card" style={{marginTop:'16px',textAlign:'center',padding:'24px',background:'rgba(30,164,101,.05)',borderColor:'rgba(30,164,101,.2)'}}>
            ...
            <a className="btn btn-primary" style={{marginTop:'12px',display:'inline-flex'}} href="/gearboxes">Ver redutores</a>
          </div>}
        </div>
      </main>
    </div>
